        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-lg sm:rounded-lg mb-6">
                <div class="p-8">
                    ...
                    <div class="mt-6 pt-4 border-t border-gray-200 dark:border-gray-700 flex flex-wrap gap-x-8 gap-y-2 text-gray-700 dark:text-gray-300">
                        @if($program->price_per_year)
                            <div><span class="font-semibold">Yearly Fee:</span> <span class="text-xl font-bold text-gray-900 dark:text-white">${{ number_format($program->price_per_year, 2) }}</span></div>
                        @endif
                        @if($program->price_per_semester)
                             <div><span class="font-semibold">Semester Fee:</span> <span class="text-xl font-bold text-gray-900 dark:text-white">${{ number_format($program->price_per_semester, 2) }}</span></div>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div x-data="{ openYear: 1 }" class="space-y-2">
                        ...
                        @empty
                             <p class="text-gray-500 dark:text-gray-400">No courses have been added for this program yet. Please check back later.</p>
                        @endforelse
                    </div>
                </div>
            </div>
